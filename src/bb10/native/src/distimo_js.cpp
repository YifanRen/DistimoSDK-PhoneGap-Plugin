 ...
 * Default constructor.
 */
DistimoJS::DistimoJS(const std::string& id) : m_id(id) {
  m_distimoMgr = new webworks::DistimoNDK(this);
}

/**
 * DistimoJS destructor.
 */
DistimoJS::~DistimoJS() {
	if (m_distimoMgr) {
		delete m_distimoMgr;
...
JSExt* onCreateObject(const string& className, const string& id) {
	if (className == "DistimoJS") {
		return new DistimoJS(id);
	}
 ...
 */
string DistimoJS::InvokeMethod(const string& command) {
	...
	if (strCommand == "openAppLink") {
		return m_distimoMgr->openAppLink(arg);
	}
...
void DistimoJS::NotifyEvent(const std::string& event) {
